/*
 * Componente "sim-i2c-trama": anatomía de un mensaje I²C. Eliges dirección
 * (7 bits), sentido R/W y un byte de datos, y recorres el mensaje segmento a
 * segmento: start, dirección + R/W, ACK del esclavo, byte de datos, ACK, stop.
 * El color indica quién controla SDA en cada tramo (maestro o esclavo) — la
 * clave de quién manda el ACK y los datos. En la Raspberry Pi 4 el bus I²C-1
 * usa GPIO2 (SDA) y GPIO3 (SCL); desde Python se maneja con smbus.SMBus(1).
 *
...
(function () {
  ...
  MPI.componentes['sim-i2c-trama'] = function (el, cfg) {
    var addr = 0x53, rw = 0, data = 0x3C;   // 0x53 = ADXL345
    ...
    el.innerHTML =
      ...
      '<div class="it-controles">' +
        '<label>Dirección (7 bits) <input type="number" class="it-addr" min="0" max="127" value="83"></label>' +
