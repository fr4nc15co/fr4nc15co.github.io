/*
 * Componente "fsm-garaje": la máquina de estados de la puerta de garaje del
 * tema 8 (problema extra), animada. Seis estados (Cerrado, Abriendo, Abierto,
 * Cerrando, Par_Abri, Par_Cerr), una sola entrada (pulsación del mando en GPIO7)
 * y dos salidas (GPIO20 = motor, GPIO21 = sentido, 1 = cierra). Cada movimiento
 * dura 30 s; aquí el tiempo corre acelerado (x10) para no aburrir.
 *
...
(function () {
  ...
  // transiciones: de -> { mando: a, tiempo: a }
  var TRANS = {
  ...
  var FLECHAS = [
    ['Cerrado', 'Abriendo', 'mando', 75, 145],
    ['Abriendo', 'Abierto', '30 s', 212, 55],
    ['Abriendo', 'Par_Abri', 'mando', 138, 95],
    ['Par_Abri', 'Cerrando', 'mando', 290, 142],
    ['Abierto', 'Cerrando', 'mando', 350, 145],
    ['Cerrando', 'Cerrado', '30 s', 212, 235],
    ['Cerrando', 'Par_Cerr', 'mando', 285, 195],
    ['Par_Cerr', 'Abriendo', 'mando', 140, 195]
  ];
  ...
  MPI.componentes['fsm-garaje'] = function (el, cfg) {
    ...
    FLECHAS.forEach(function (f) {
      ...
      svg.push('<line x1="' + a.x + '" y1="' + a.y + '" x2="' + b.x + '" y2="' + b.y +
               '" stroke="var(--txt-tenue,#6a737d)" stroke-width="1.5" marker-end="url(#fle)"/>');
    ...
    Object.keys(ESTADOS).forEach(function (n) {
      ...
      svg.push('<g class="fsm-nodo" data-estado="' + n + '">' +
        ...
        '<text x="' + e.x + '" y="' + (e.y + 4) + '" text-anchor="middle" font-size="12" fill="var(--txt,#d4d4d4)">' + n + '</text></g>');
    });
    svg.push('</svg>');
      ...
      '<div class="fsm-cuerpo">' +
        '<div class="fsm-izq">' + svg.join('') +
          '<div class="fsm-controles">' +
            '<button type="button" class="fsm-mando">📡 Pulsación del mando (GPIO7)</button>' +
        ...
        '<div class="fsm-der">' +
          ...
          '<div class="fsm-salidas">' +
            '<span class="fsm-led" data-led="motor">GPIO20 · motor</span>' +
            '<span class="fsm-led" data-led="sentido">GPIO21 · sentido (1 = cierra)</span>' +
          '</div>' +
    ...
    function arrancaMovimiento() {
      ...
      timer = setInterval(function () {
        if (estado === 'Abriendo') {
          ...
          if (pos >= 100) { pos = 100; para(); estado = 'Abierto'; pintar('Fin de los 30 s: transición automática <strong>Abriendo → Abierto</strong> (saltó el temporizador de 30 s).'); return; }
    ...
    el.querySelector('.fsm-mando').addEventListener('click', function () {
      ...
      if (estado === 'Abriendo' || estado === 'Cerrando') {
        ...
        pintar('Pulsación: <strong>' + ant + ' → ' + sig + '</strong>. ' +
          (ant.indexOf('Par_') === 0 ? 'Se reanuda en sentido contrario aprovechando que t<sub>abrir</sub> = 30 − t<sub>cerrar</sub>.' :
