...
<section>
  ...
  <table class="tabla-datos tabla-modos">
    ...
    <tbody>
      <tr><td><strong>Temporizador (normal)</strong></td><td>0</td><td>0</td><td>Reloj interno PBCLK</td><td>Generar retardos y señales/interrupciones periódicas contando hasta <code class="reg">PRx</code>. <em>Es el uso habitual.</em></td></tr>
      <tr><td><strong>Contador de eventos</strong></td><td>1</td><td>0</td><td>Flancos del pin <code class="reg">TxCK</code></td><td>Contar pulsos externos (el pin <strong>es</strong> el reloj). Ej.: contar 7 pulsaciones.</td></tr>
...
<section>
  ...
  <p class="nota">El −1 es porque la cuenta empieza en 0. Y <code class="reg">PR</code> debe caber en 16 bits (≤ 65535) o en 32 bits (≤ 4&nbsp;294&nbsp;967&nbsp;295) si el timer está encadenado.</p>
  ...
  <pre><code class="lang-c">int main(void) {
    ...
    while (IFS0bits.T1IF == 0) ;   // Esperar el fin de cuenta
    // ...
}</code></pre>
  ...
  <h3>Ejemplo 3 — retardo largo con un solo timer de 16 bits</h3>
...
<section>
  <h2>4.4 Ejercicio de diseño de drivers</h2>
