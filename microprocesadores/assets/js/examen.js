...
(function () {
  ...
  function montarTest(el, n) {
    ...
    gaEvent('examen_inicio', { total_preguntas: elegidas.length, site: 'microprocesadores' });
    ...
    el.querySelector('.ex-corregir').addEventListener('click', function () {
      ...
      gaEvent('examen_resultado', { aciertos: aciertos, total: items.length, nota: sobre10, site: 'microprocesadores' });
      this.disabled = true;
      el.querySelector('.ex-nota').scrollIntoView({ block: 'center', behavior: 'smooth' });
    });
    el.querySelector('.ex-otro').addEventListener('click', function () { gaEvent('examen_reintentar', { site: 'microprocesadores' }); montarTest(el, n); window.scrollTo(0, 0); });
  }
